...
describe Api::V1::EquationsController do
  describe 'POST #linear' do
    it 'with valid params' do
      post :linear, params: { coefficients: [2, -10] }, as: :json

      expect(response).to have_http_status :ok
      expect(response.body).to eq({ root: [5.0] }.to_json)
    end
    ...
    context 'with invalid params' do
      ...
      it 'when wrong type of coefficients' do
        post :linear, params: { coefficients: 'coefficients' }, as: :json
      ...
      it 'when wrong number of coefficients' do
        post :linear, params: { coefficients: [1, 2, 3] }, as: :json
  ...
  describe 'POST #quadratic' do
    it 'with valid params' do
      post :quadratic, params: { coefficients: [1, -8, 12] }, as: :json

      expect(response).to have_http_status :ok
      expect(response.body).to eq({ roots: [6.0, 2.0] }.to_json)
    end
    ...
    context 'with invalid params' do
      ...
      it 'when wrong type of coefficients' do
        post :quadratic, params: { coefficients: 'coefficients' }, as: :json
      ...
      it 'when wrong number of coefficients' do
        post :quadratic, params: { coefficients: [1, 2] }, as: :json
      ...
      it 'when wrong value of `a` coefficient' do
        post :quadratic, params: { coefficients: [0, 2, 3] }, as: :json
